...
int scoreBoard(Grid<char>& board, Lexicon& lex) {
    ...
    for (int r = 0; r < board.numRows(); r++) {
        for (int c = 0; c < board.numCols(); c++) {
            char BoggleLetter = board[r][c];    //find all words starting on this square
            //if the next letter is inBounds and is a prefix of a word in the lexicon, continue
            // if each direction yields no prefix word, youre done with that path
            // maybe start the helper function here and pass in the char location
...
Set<string> adjacentChars(GridLocation start) {
    //generate neighbors of char while checking if they are in bounds, not apart of set of visited locations,
    string curString = "";
    for (int r = -1; r < start.row + 2; r++) {
        for (int c = -1; c < start.col + 2; c++) {
            GridLocation adj;
                adj.col = c;
                adj.row = r;
