...
//            pq.enqueue(point);
//        } else if (pq.size() == k) {
//            if (pq.peek().priority < point.priority) {
//                pq.dequeue();
//                pq.enqueue(point);
//            }
//        }
//    }
//    //cout << "the size of pq" << pq.size() << endl;
...
STUDENT_TEST("Testing with random values") {
    Vector <DataPoint> testing1212 = {{ "A", 1 }, { "B", 2 }, { "C", 3 }, { "D", 4 }, { "D", -4 }, { "C", -30 }, { "B", 200 }};
    stringstream butter = asStream(testing1212);
    Vector<DataPoint> expect = {{ "B", 200 }};
    EXPECT_EQUAL(topK(butter, 1), expect);
}

STUDENT_TEST("Provided Test: Time operation top-k") {
